...
from expo.evaluation import evaluate, evaluate_diffusion
...
flags.DEFINE_boolean(
    "expo", False, "Whether to run EXPO."
)
flags.DEFINE_boolean(
...
def main(_):
    ...
    for i in tqdm.tqdm(
        ...
        if i >= FLAGS.start_training:
            ...
            # Success buffer BC regularization (disabled - requires is_success/score keys in buffer)
        ...
        if i % FLAGS.eval_interval == 0:
            if not FLAGS.expo:
                ...
                eval_info, agent = evaluate_diffusion(
                    agent,
                    eval_env,
                    num_episodes=FLAGS.eval_episodes,
                    save_video=FLAGS.save_video,
